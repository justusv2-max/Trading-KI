...
# Warmup Closes - nur beim allerersten Start ohne Cache
...
if cache:
    ...
    # Bei Neustart: nur letzte 25 Closes aus Cache - KEINE Warmup Closes
    if "shared_daily" in cache and len(cache["shared_daily"]) >= 21:
        ...
        # Noch nicht genug im Cache - Warmup als Basis
        SHARED_DAILY = deque(WARMUP_CLOSES, maxlen=30)
        print(f"Daily Closes: Warmup Basis ({len(SHARED_DAILY)} Closes)")
else:
    print("Erster Start - Warmup Closes geladen!")
    BUFFERS = {k: deque(DEFAULT_BARS[k], maxlen=300) for k in PARAMS}
    # Erster Start: Warmup Closes als Basis
    SHARED_DAILY = deque(WARMUP_CLOSES, maxlen=30)
    print(f"Daily Closes: {len(SHARED_DAILY)} Warmup Closes")

mom_init = (list(SHARED_DAILY)[-1] - list(SHARED_DAILY)[-21]) / list(SHARED_DAILY)[-21] * 100 if len(SHARED_DAILY) >= 21 else None
print(f"Momentum beim Start: {mom_init:.1f}%" if mom_init else "Momentum: Noch nicht berechenbar")

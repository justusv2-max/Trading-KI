...
PARAMS = {
    "CL_H1": {
        "momentum_min_abs": 0.50, "stop_buffer": 0.10, "max_risk": 5.00,
        "crv": 2.0, "atr_max": 1.00, "atr_pct": False,
        "vol_ma_len": 20, "mom_threshold": 15.0, "max_pb_bars": 72,
        "sessions": ["Europa", "US"], "excl_hours": [15],
    },
    "CL_M30": {
        "momentum_min_abs": 0.50, "stop_buffer": 0.10, "max_risk": 5.00,
        "crv": 2.0, "atr_max": 1.00, "atr_pct": False,
        "vol_ma_len": 20, "mom_threshold": 15.0, "max_pb_bars": 144,
        "sessions": ["Europa", "US"], "excl_hours": [15, 19],
    },
    "ES_M30": {
        "momentum_min_pct": 0.0071, "stop_buffer_pct": 0.0014, "max_risk_pct": 0.071,
        "crv": 1.0, "atr_max_pct": 0.0143, "atr_pct": True,
        "vol_ma_len": 20, "mom_threshold": 10.0, "max_pb_bars": 144,
        "sessions": ["Europa", "US"], "excl_hours": [15],
    },
}
...
def check_signal(inst, bar):
    ...
    pr = sum(b["close"] for b in bars[-20:]) / min(20, len(bars))
    if p["atr_pct"]:
        if atr > pr * p["atr_max_pct"]: return None
        mm = pr * p["momentum_min_pct"]; sb = pr * p["stop_buffer_pct"]; mr = pr * p["max_risk_pct"]
    else:
        if atr > p["atr_max"]: return None
        mm = p["momentum_min_abs"]; sb = p["stop_buffer"]; mr = p["max_risk"]
    mom = calc_mom(DAILY_CLOSES[inst])
    if mom is not None and p["mom_threshold"] and abs(mom) > p["mom_threshold"]: return None
    for i in range(2, min(12, len(bars))):
        liq = bars[-i-1]["low"]; extr = bars[-i]["low"]
        if extr < liq and c["close"] >= liq + mm:
            en = liq; st = extr - sb; rk = en - st
            if 0 < rk <= mr:
                s["in_trade"] = True; s["bars_since"] = 0
                return {"direction": "LONG", "entry": en, "stop": st, "target": en+rk*p["crv"]}
    for i in range(2, min(12, len(bars))):
        liq = bars[-i-1]["high"]; extr = bars[-i]["high"]
        if extr > liq and c["close"] <= liq - mm:
            en = liq; st = extr + sb; rk = st - en
            if 0 < rk <= mr:
                s["in_trade"] = True; s["bars_since"] = 0
                return {"direction": "SHORT", "entry": en, "stop": st, "target": en-rk*p["crv"]}
    return None
...
def webhook(instrument):
    ...
    try:
        ...
        bar = {"timestamp": data.get("timestamp", datetime.now().timestamp()),
    ...
    if sig:
        names = {"CL_H1": "CL H1", "CL_M30": "CL M30", "ES_M30": "ES M30"}
        crv = PARAMS[inst]["crv"]
        d = sig["direction"]
...
def home():
    return jsonify({"status": "running", "system": "CL H1 (2R) + CL M30 (2R) + ES M30 (1R)"})
